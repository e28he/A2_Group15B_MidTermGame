...
function drawStartOverlay() {
  ...
  text(
    "🔥  Tip: Keep a rhythm to build combo bonus",
    startX,
    startY + spacing * 5.2,
  );
...
function updateArmReach() {
  ...
  const reachLimit =
    game.state === STATES.SERVE_DRINK ? width * 0.55 : width * 0.45;
...
function station(name) {
  ...
  const map = {
    ...
    serve: { x: width * 0.88, y: y - 110, r: 58, label: "Serve" },
  };
  return map[name];
}
...
function drawScene() {
  ...
  stroke(170, 191, 219);
  strokeWeight(4);
  line(186, 120, 186, 260);
  line(96, 190, 276, 190);
  noStroke();

  fill(212, 176, 133);
  rect(width - 370, 132, 250, 16, 8);
  fill(140, 176, 216);
  rect(width - 350, 102, 22, 30, 6);
  fill(248, 228, 190);
  ellipse(width - 306, 118, 26, 26);
...
function drawArms() {
  ...
  for (const arm of arms) {
    ...
    let armColor = color(158, 148, 255); // Changed to match the bottom of the gradient
...
function drawHeader() {
  ...
  textSize(22);
  textStyle(BOLD);
  text("☕ Shift Status", 45, 30);
  textStyle(NORMAL);

  fill(240, 245, 255);
  rect(260, 28, 130, 30, 15);
  fill(100, 140, 200);
  textSize(16);
  textStyle(BOLD);
  ...
  fill(40, 60, 90);
  textStyle(BOLD);
  text(`${game.ordersDone} / ${GAME.targetOrders}`, 105, 65);
  textStyle(NORMAL);
  fill(80, 100, 130);
  text(`Score:`, 180, 65);
  fill(40, 60, 90);
  textStyle(BOLD);
  text(`${floor(game.score)}`, 230, 65);
  ...
  fill(255, 150, 80);
  textStyle(BOLD);
  text(`x${game.combo}`, 105, 90);
  textStyle(NORMAL);
  fill(80, 100, 130);
  text(`Mistakes:`, 180, 90);
  fill(220, 100, 100);
  textStyle(BOLD);
  text(`${game.mistakes}`, 250, 90);
...
function drawTangleBar(x, y, w, h) {
  ...
  const cA = color(131, 197, 255);
  const cB = color(255, 180, 100);
  const cC = color(255, 100, 100);
...
function drawQueuePanel() {
  setShadow(15, "rgba(0,0,0,0.1)");
  fill(255, 255, 255, 240);
  rect(width - 300, height - 214, 280, 194, 20);
  clearShadow();
...
function drawGuidePanel() {
  ...
  if (game.state === STATES.SERVE_DRINK) {
    msg = game.selectedArm
      ? "Bring cup to Serve Area"
      : "Choose tentacle to serve (W/A/S/D)";
  }
...
function challengeHint() {
  ...
  if (game.challenge.type === "hold")
    return `${labelStep(step)}: hold mouse or SPACE in safe zone`;
...
function drawGameOver() {
  ...
  rect(width * 0.3, height * 0.15, width * 0.4, height * 0.7, 24);
  clearShadow();

  fill(235, 242, 255);
  rect(width * 0.3, height * 0.15, width * 0.4, 90, 24, 24, 0, 0);
  ...
  textSize(44);
  textStyle(BOLD);
  text("Shift Complete", width * 0.5, height * 0.21);
  textStyle(NORMAL);
  ...
  textSize(20);
  textStyle(BOLD);
  text(`Orders: ${game.ordersDone}`, width * 0.5, height * 0.5);
  text(`Final Score: ${floor(game.score)}`, width * 0.5, height * 0.55);
  textStyle(NORMAL);
  textSize(18);
  fill(90, 110, 150);
  text(`Mistakes: ${game.mistakes}`, width * 0.5, height * 0.6);
  ...
  textSize(18);
  textStyle(BOLD);
  text("Press SPACE to Restart", width * 0.5, height * 0.78);
  textStyle(NORMAL);
}
